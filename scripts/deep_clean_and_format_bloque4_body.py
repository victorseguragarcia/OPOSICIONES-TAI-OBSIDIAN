...
print("=" * 70)
print("🧼 LIMPIEZA PROFUNDA Y FORMATEO - TEMAS COMPLETOS BLOQUE 4")
print("=" * 70)
...
ACADEMIA_NOISE = [
    ...
    r"Campus Virtual",
    r"Administración del Sistema\s+Operativo y software de Base[^\n]+",
    r"Servicios de Directorio[^\n]+",
    r"Virtualización y Cloud[^\n]+",
    r"Centros de Proceso de Datos[^\n]+",
    r"Copias de Seguridad y Continuidad[^\n]+",
    r"Medios de Transmisión[^\n]+",
    r"Redes Locales Ethernet[^\n]+",
    r"Protocolo IP y Subnetting[^\n]+",
    r"Protocolos de Transporte y Puertos[^\n]+",
    r"Seguridad de la Información y ENS[^\n]+",
    r"Seguridad Perimetral y Firewalls[^\n]+"
]

def clean_and_format_b4_topic(content):
    ...
    m = re.search(r"(A lo largo de la historia|El concepto de|En este tema|El software es|Un sistema operativo|El servicio de directorio|Active Directory|La virtualización|Un Centro de Proceso|Las copias de seguridad|Los medios de transmisión|El protocolo Ethernet|El direccionamiento IP|El protocolo TCP|La seguridad de la información|Un cortafuegos|El Esquema Nacional)", body_text)
    if m:
        start_pos = m.start()
...
for tnum in range(1, 11):
    ...
    cleaned_content = clean_and_format_b4_topic(raw_content)
    fpath.write_text(cleaned_content, encoding="utf-8")
    print(f"  [Cleaned & Formatted Bloque 4] {fname} ({len(cleaned_content.splitlines())} líneas)")
